...
function normalizePhotoUrl(rawUrl) {
  ...
  if (driveFileMatch && driveFileMatch[1]) return `https://drive.google.com/thumbnail?id=${driveFileMatch[1]}&sz=w1200`;
  const driveIdMatch = url.match(/[?&]id=([^&]+)/i);
  if (driveIdMatch && /drive\.google\.com/i.test(url)) return `https://drive.google.com/thumbnail?id=${driveIdMatch[1]}&sz=w1200`;
  return url;
}
